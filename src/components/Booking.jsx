...
const Booking = () => {
    ...
    const [vehicleChoice, setVehicleChoice] = useState('previous'); // 'previous' or 'another'
    ...
    // Update form if user or booking type changes
    useEffect(() => {
        if (bookingFor === 'myself') {
            setFormData(prev => ({
                ...
                email: user?.email || '',
                phone: user?.phone || '',
                vehicleType: vehicleChoice === 'previous' ? (prevVehicles[0] || '') : ''
            }));
        } else {
            setFormData(prev => ({
                ...prev,
            ...
            <div className="section-container">
                <div className="booking-grid">
                    ...
                    >
                        ...
                        <p className="section-description" style={{ color: '#888', maxWidth: '500px' }}>
                        ...
                        <div className="booking-benefits" style={{ marginTop: '2.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            <div className="benefit-item" style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                                <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: 'rgba(201,169,106,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-gold)' }}>
                                    <FiTruck size={24} />
                                </div>
                                <div>
                                    <h4 style={{ color: '#fff', margin: 0 }}>Executive Mobile Service</h4>
                                    <p style={{ color: '#666', fontSize: '0.85rem', margin: 0 }}>We come to your home or office.</p>
                                </div>
                            </div>
                            <div className="benefit-item" style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                                <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: 'rgba(201,169,106,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-gold)' }}>
                                ...
                                <div>
                                    <h4 style={{ color: '#fff', margin: 0 }}>Strict Punctuality</h4>
                                    <p style={{ color: '#666', fontSize: '0.85rem', margin: 0 }}>On-time arrival, guaranteed.</p>
                                </div>
                            </div>
                        </div>
                    </motion.div>
                    ...
                    >
                        <form onSubmit={handleSubmit}>
                            ...
                            {/* Vehicle Selection - Only for 'myself' */}
                            {bookingFor === 'myself' && prevVehicles.length > 0 && (
                                <div style={{ marginBottom: '1.5rem' }}>
                                    <label style={{ color: '#888', fontSize: '0.75rem', marginBottom: '0.5rem', display: 'block' }}>Select Vehicle</label>
                                    <div style={{ display: 'flex', gap: '1rem' }}>
                                        <button
                                            type="button"
                                            onClick={() => setVehicleChoice('previous')}
                                            style={{
                                                flex: 1, padding: '0.8rem', borderRadius: '10px', fontSize: '0.85rem', cursor: 'pointer',
                                                border: '1px solid', borderColor: vehicleChoice === 'previous' ? 'var(--color-gold)' : '#222',
                                                background: vehicleChoice === 'previous' ? 'rgba(201,169,106,0.05)' : 'transparent',
                                                color: vehicleChoice === 'previous' ? '#fff' : '#666'
                                            }}
                                        >
                                            Previous Vehicle
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => setVehicleChoice('another')}
                                            style={{
                                                flex: 1, padding: '0.8rem', borderRadius: '10px', fontSize: '0.85rem', cursor: 'pointer',
                                                border: '1px solid', borderColor: vehicleChoice === 'another' ? 'var(--color-gold)' : '#222',
                                                background: vehicleChoice === 'another' ? 'rgba(201,169,106,0.05)' : 'transparent',
                                                color: vehicleChoice === 'another' ? '#fff' : '#666'
                                            }}
                                        >
                                            Another Vehicle
                                        </button>
                            ...
                            {/* Vehicle Input */}
                            <div style={{ marginBottom: '1.5rem' }}>
                                {bookingFor === 'myself' && vehicleChoice === 'previous' ? (
                                    <div style={{ position: 'relative' }}>
                                        <select
                                            name="vehicleType"
                                    ...
                                    <div style={{ position: 'relative' }}>
                                        <input
                                            ...
                                            onChange={handleChange}
                                            required
                                            placeholder="e.g. 2024 Mercedes-AMG G63"
                            ...
                            {/* Location with Google Map option */}
                            <div style={{ marginBottom: '1.5rem' }}>
                                ...
                                <div style={{ display: 'flex', gap: '0.8rem' }}>
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => {
                                            // Simulate a map picker entry
                                            const mockAddress = "Bole Medhanialem, Street 14";
                                        ...
                                        style={{
                                            ...
                                            color: 'var(--color-gold)', display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', transition: '0.2s'
                                        }}
                                    >
                                        <FiMap /> <span style={{ fontSize: '0.8rem' }}>Map Pin</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div style={{ marginBottom: '2rem' }}>
                                ...
                                <div style={{ position: 'relative' }}>
                                    <textarea
                                        ...
                                        onChange={handleChange}
                                        placeholder="Specific vehicle concerns, entry codes, etc."
                                        rows="3"
